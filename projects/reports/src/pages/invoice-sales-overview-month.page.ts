...
@Component({
  ...
  template: `
    <app-layout-sidenav
      ...
      <ng-template #body>
        <app-date-range view="year" (export)="exportReport()" (reload)="getSalesOverview($event)"></app-date-range>
        <app-invoice-sales-overview
          *ngIf="(reportState.loadMonthInvoiceSales | async)===false"
          [sales]="reportState.invoiceSalesByMonth | async"></app-invoice-sales-overview>
        <app-data-not-ready
          [height]="100"
          *ngIf="(reportState.loadMonthInvoiceSales | async)===true"
          [isLoading]="(reportState.loadMonthInvoiceSales | async)===true"></app-data-not-ready>
      </ng-template>
    </app-layout-sidenav>
  `,
