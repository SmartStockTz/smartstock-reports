...
export class ReportService {
  ...
  async getProductPerformanceReport(from: Date, to: Date): Promise<CashSalesPerformanceProductModel[]> {
    const shop = await this.userService.getCurrentShop();
    return functions(shop.projectId).request(
  ...
  async getCategoryPerformanceReport(from: Date, to: Date): Promise<CashSalesPerformanceCategoryModel[]> {
    const shop = await this.userService.getCurrentShop();
    return functions(shop.projectId).request(
  ...
  async getSellerPerformanceReport(from: Date, to: Date): Promise<CashSalesPerformanceSellerModel[]> {
    const shop = await this.userService.getCurrentShop();
    return functions(shop.projectId).request(
